...
function fallbackCopy(text) {
  ...
  try {
    ...
    if (success) {
      console.info('Fallback copy successful (execCommand)');
    } else {
 ...
 * @param {number} duration - Duration in ms
 */
export function showCopyFeedback(element, successText = '✓ Copied!', originalText = null, duration = 2000) {
 ...
 */
export async function handleCopyButton(button, textOrGetter, options = {}) {
  const {
 ...
 * @returns {Promise<boolean>} - Success status
 */
export async function copyWithToast(text, options = {}) {
  const {
    successMessage = 'Copied to clipboard!',
